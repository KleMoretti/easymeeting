...
public class MeetingMember implements Serializable {
	...
	public String toString() {
		return ": " + (meetingId == null ? "空" : meetingId) + ",: " + (userId == null ? "空" : userId) + ",: " + (nickName == null ? "空" : nickName) + ",: " + (lastJoinTime == null ? "空" : DataUtils.format(lastJoinTime,DateTimePatternEnum.YYYY_MM_DD_HH_MM_SS.getPattern())) + ",: " + (status == null ? "空" : status) + ",: " + (memberType == null ? "空" : memberType) + ",: " + (meetingStatus == null ? "空" : meetingStatus); 
	}
}